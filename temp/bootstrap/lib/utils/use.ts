...
import { listen } from 'rease'

export const useGetNodeBeforeCreated = (
  storeForNode: ReaseStore<any>
) => (ctx: TypeReaseContext): void => { storeForNode.$ = ctx.node }
...
export const useOnPan = <C extends readonly [] | {} | undefined = undefined>(
  ...
  detail?: C
    ...
    const unlisteners = [
      listen(node, 'pointerdown', (e: PointerEvent, c) => {
        c.is = true
        c.ox = c.oy = 0
      ...
      listen(document, 'pointermove', (e: PointerEvent, c) => {
        if (c.is) {
      ...
      listen(document, 'pointerup', (e: PointerEvent, c) => {
        if (c.is) {
          c.is = false
          cb({
            type  : 'end',
